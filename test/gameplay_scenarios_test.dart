...
void main() {
  ...
  group('Required Role Composition Tests', () {
    test('Game requires at least 1 Dealer', () {
      // Setup: No dealers
      gameEngine.addPlayer('Alice', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('Bob', role: roleRepository.getRoleById('medic'));
      gameEngine.addPlayer('Charlie', role: roleRepository.getRoleById('wallflower'));
    ...
    test('Game requires at least 1 Party Animal', () {
      ...
      gameEngine.addPlayer('Wallflower1', role: roleRepository.getRoleById('wallflower'));
      gameEngine.addPlayer('Bouncer1', role: roleRepository.getRoleById('bouncer'));
    ...
    test('Game requires at least 1 Wallflower', () {
      ...
      gameEngine.addPlayer('PA2', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('Medic1', role: roleRepository.getRoleById('medic'));
    ...
    test('Dealers cannot have majority at start', () {
      gameEngine.addPlayer('Dealer1', role: roleRepository.getRoleById('dealer'));
      gameEngine.addPlayer('Dealer2', role: roleRepository.getRoleById('dealer'));
      gameEngine.addPlayer('Dealer3', role: roleRepository.getRoleById('dealer'));
      gameEngine.addPlayer('PA1', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('WF1', role: roleRepository.getRoleById('wallflower'));
      ...
      final dealerCount = enabledPlayers.where((p) => p.role.id == 'dealer').length;
  ...
  group('Win Condition Scenarios', () {
    ...
    test('Neutral (Club Manager) wins if they survive to the end', () {
      ...
      gameEngine.addPlayer('PA1', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('WF1', role: roleRepository.getRoleById('wallflower'));
      gameEngine.addPlayer('CM1', role: roleRepository.getRoleById('club_manager'));
      ...
      final clubManager = gameEngine.players.firstWhere((p) => p.role.id == 'club_manager');
  ...
  group('Role-Specific Ability Tests', () {
    ...
    test('Seasoned Drinker has extra lives based on dealer count', () {
      gameEngine.addPlayer('Dealer1', role: roleRepository.getRoleById('dealer'));
      gameEngine.addPlayer('Dealer2', role: roleRepository.getRoleById('dealer'));
      gameEngine.addPlayer('SD1', role: roleRepository.getRoleById('seasoned_drinker'));
      gameEngine.addPlayer('PA1', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('WF1', role: roleRepository.getRoleById('wallflower'));

      final sd = gameEngine.players.firstWhere((p) => p.role.id == 'seasoned_drinker');

      // Set lives based on dealer count
      final dealerCount = gameEngine.players.where((p) => p.role.id == 'dealer').length;
    ...
    test('Ally Cat has 9 lives', () {
      ...
      final allyCat = gameEngine.players.firstWhere((p) => p.role.id == 'ally_cat');
      allyCat.initialize();
    ...
    test('Second Wind can convert to Dealer', () {
      gameEngine.addPlayer('Dealer1', role: roleRepository.getRoleById('dealer'));
      gameEngine.addPlayer('SW1', role: roleRepository.getRoleById('second_wind'));
      gameEngine.addPlayer('PA1', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('WF1', role: roleRepository.getRoleById('wallflower'));
      gameEngine.addPlayer('Medic1', role: roleRepository.getRoleById('medic'));

      final secondWind = gameEngine.players.firstWhere((p) => p.role.id == 'second_wind');
  ...
  group('Player Count Variation Tests', () {
    ...
    test('Valid 15-player game with 3 dealers', () {
      ...
      for (var i = 1; i <= 4; i++) {
        gameEngine.addPlayer('PA$i', role: roleRepository.getRoleById('party_animal'));
      }
      gameEngine.addPlayer('WF1', role: roleRepository.getRoleById('wallflower'));
      gameEngine.addPlayer('Medic1', role: roleRepository.getRoleById('medic'));
      gameEngine.addPlayer('Bouncer1', role: roleRepository.getRoleById('bouncer'));
      ...
      gameEngine.addPlayer('TeaSpiller1', role: roleRepository.getRoleById('tea_spiller'));
      gameEngine.addPlayer('Lightweight1', role: roleRepository.getRoleById('lightweight'));
      gameEngine.addPlayer('Silver Fox1', role: roleRepository.getRoleById('silver_fox'));
  ...
  group('Late Join Scenarios', () {
    test('Player can join mid-game with available role', () {
      ...
      // Move to day phase
      gameEngine.skipToNextPhase(); // -> day
    ...
    test('Late joiner cannot get a role that already appeared', () {
      ...
      expect(roleIds, isNot(contains('wallflower'))); // Unique role already used
      expect(roleIds, contains('party_animal')); // Can repeat (always available)
    ...
    test('Late joiner becomes active on next night', () {
      ...
      // Move to next night
      gameEngine.skipToNextPhase(); // -> night
  ...
  group('Unique Role Enforcement', () {
    ...
    test('Can assign multiple Party Animals', () {
      gameEngine.addPlayer('PA1', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('PA2', role: roleRepository.getRoleById('party_animal'));
      gameEngine.addPlayer('PA3', role: roleRepository.getRoleById('party_animal'));

      final paCount = gameEngine.players.where((p) => p.role.id == 'party_animal').length;
      expect(paCount, 3);
    });
  });
  ...
  group('Edge Case Scenarios', () {
    ...
    test('Dealers can win even with neutrals alive', () {
      ...
      // Club Manager should still be alive
      final cm = gameEngine.players.firstWhere((p) => p.role.id == 'club_manager');
