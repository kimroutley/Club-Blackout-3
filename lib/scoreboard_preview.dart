...
class ScoreboardPreviewScreen extends StatefulWidget {
  ...
  @override
  State<ScoreboardPreviewScreen> createState() => _ScoreboardPreviewScreenState();
}
...
class _ScoreboardPreviewScreenState extends State<ScoreboardPreviewScreen> {
  ...
  void _setupFakeGame() {
    ...
    // --- Create Roles ---
    final dealerRole = Role(id: 'dealer', name: 'The Dealer', alliance: 'The Dealers', type: 'aggressive', description: 'desc', nightPriority: 1, assetPath: '', colorHex: '#FF00FF', choices: [], hasBinaryChoiceAtStart: false);
    final partyRole = Role(id: 'party_animal', name: 'Party Animal', alliance: 'The Party Animals', type: 'passive', description: 'desc', nightPriority: 0, assetPath: '', colorHex: '#FFDAB9', choices: [], hasBinaryChoiceAtStart: false);
    final medicRole = Role(id: 'medic', name: 'The Medic', alliance: 'The Party Animals', type: 'defensive', description: 'desc', nightPriority: 1, assetPath: '', colorHex: '#FF0000', choices: ['PROTECT'], hasBinaryChoiceAtStart: true);
    ...
    engine.voteChanges.add(VoteCast(day: 1, voterId: 'p1', targetId: 'p2', timestamp: DateTime.now(), sequence: 0));
    engine.voteChanges.add(VoteCast(day: 1, voterId: 'p1', targetId: 'p3', timestamp: DateTime.now(), sequence: 1));
    engine.voteChanges.add(VoteCast(day: 1, voterId: 'p1', targetId: 'p4', timestamp: DateTime.now(), sequence: 2));

    // 2. "Public Enemy #1": Tim gets targeted at night
    engine.nightHistory.add({'kill': 'p2', 'protect': 'p2'}); // Targeted + Saved
    ...
    engine.voteHistory.add(VoteCast(day: 1, voterId: 'p1', targetId: 'p4', timestamp: DateTime.now(), sequence: 10)); // Ian -> Vince
    engine.voteHistory.add(VoteCast(day: 1, voterId: 'p3', targetId: 'p4', timestamp: DateTime.now(), sequence: 11)); // Judy -> Vince
    engine.voteHistory.add(VoteCast(day: 1, voterId: 'p2', targetId: 'p4', timestamp: DateTime.now(), sequence: 12)); // Tim -> Vince
    // Vince -> Judy
    engine.voteHistory.add(VoteCast(day: 1, voterId: 'p4', targetId: 'p3', timestamp: DateTime.now(), sequence: 13));
    ...
    engine.voteHistory.add(VoteCast(day: 2, voterId: 'p1', targetId: 'p5', timestamp: DateTime.now(), sequence: 20));
