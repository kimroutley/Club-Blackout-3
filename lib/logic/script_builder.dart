...
class ScriptBuilder {
  ...
  static List<ScriptStep> buildNightScript(List<Player> players, int dayCount) {
    ...
    try {
      ...
      final others = players
          ...
          .toList();
      final hasWonCondition =
          others.isNotEmpty && others.every((p) => p.hasRumour);
    ...
    if (dayCount == 0) {
      ...
      if (hasClinger) {
        steps.add(
          const ScriptStep(
            ...
            instructionText:
                "This player becomes your partner. You must vote exactly as they vote. If they die, you die. Write down their choice.",
        ...
        steps.add(
          const ScriptStep(
            ...
            readAloudText:
                "Clinger, you will now see your obsession's role card.\n\nClinger, close your eyes.",
            instructionText:
                "Show the Clinger their obsession's role card for 10 seconds so they can mimic their partner's behavior. Then they close eyes.",
      ...
      if (hasCreep) {
        steps.add(
          const ScriptStep(
            ...
            instructionText:
                "Wait for them to point. You will inherit this player's role if that player dies. Write down their choice.",
            actionType: ScriptActionType.selectPlayer,
            roleId: 'creep',
          ),
        );
        steps.add(
          const ScriptStep(
            ...
            readAloudText:
                "Creep, you will now see your target's role card.\n\nCreep, close your eyes.",
            instructionText:
                "Show the Creep their target's role card for 10 seconds so they can learn how to pretend to be that role. Then they close eyes.",
            actionType: ScriptActionType.showInfo,
            roleId: 'creep',
          ),
        );
      ...
      if (hasMedic) {
        steps.add(
          const ScriptStep(
            ...
            instructionText:
                "PROTECT: You protect one player each night. REVIVE: You can revive one dead player once per game. Write down their choice. Then they close eyes.",
      ...
      steps.add(
        const ScriptStep(
          ...
          readAloudText:
              "Setup is complete. Night 1 begins now. Keep your eyes closed.",
    ...
    for (var p in players) {
      ...
      if (p.role.id == 'whore' || p.role.id == 'silver_fox') {
        // Temporarily disabled per host request
        continue;
      }
    ...
    for (var role in orderedRoles) {
      ...
      if (role.id == 'roofi') {
        final activeRoofi = players
            .where((p) => p.isActive && p.role.id == 'roofi')
            .toList();
      ...
      if (role.id == 'messy_bitch' && messyBitchWithKill != null) {
        steps.addAll([
          ScriptStep(
            ...
            instructionText:
                "The Messy Bitch has reached her win condition and can now kill someone directly (one time only).",
          ...
          ScriptStep(
            ...
            instructionText:
                "This is a one-time ability after reaching her win condition. If you choose not to use it, you pass.",
            actionType: ScriptActionType.selectPlayer,
            roleId: 'messy_bitch_kill',
          ),
          ScriptStep(
    ...
    // 3. End Night - Club Closed (single screen, no extra "night is over" dialog)
    steps.add(
      const ScriptStep(
        id: 'club_closed',
        title: 'CLUB IS CLOSED',
        readAloudText:
            "Lights up, troublemakers — open your eyes. Time for the morning report and a vote.",
        instructionText:
            "Let everyone open their eyes, then proceed directly into the Day Phase.",
        isNight: false,
  ...
  static List<ScriptStep> buildDayScript(
    ...
    if (players.any(
      (p) => p.secondWindPendingConversion && !p.secondWindConverted,
    )) {
      daySteps.add(
        ScriptStep(
          ...
          readAloudText:
              "Dealers, I have secret information regarding your victim... I will now approach you for a decision.",
          instructionText:
              "Approach the Dealers. Ask them secretly if they wish to convert The Second Wind instead of killing them. Show them the options on screen.",
          isNight: false,
    ...
    daySteps.add(
      ScriptStep(
        ...
        title: 'BEGIN DAY PHASE',
        readAloudText: morningAnnouncement,
        instructionText:
            "Open the Day Phase dialog to show the Morning Report and run voting.",
        isNight: false,
  ...
  static List<ScriptStep> _buildDealerSteps(
    ...
    final aliveDealers = players
        .where((p) => p.isActive && p.role.id == 'dealer')
        .toList();
      ...
      if (d.blockedKillNight == dayCount) {
        return const [
          ScriptStep(
            ...
            instructionText:
                "The only Dealer is incapacitated and cannot act tonight.",
            actionType: ScriptActionType.showInfo,
            roleId: 'dealer',
          ),
        ];
    ...
    final hasWallflower = players.any(
      (p) => p.isActive && p.role.id == 'wallflower',
    );
    ...
    return [
      ScriptStep(
        ...
        instructionText:
            "Wait for them to acknowledge, then wait for a consensus pointing.",
        actionType: ScriptActionType.selectPlayer,
        roleId: 'dealer',
      ),
      // SECOND WIND - Conversion Voting (only if Second Wind was killed)
      if (players.any(
        (p) => p.role.id == 'second_wind' && p.secondWindPendingConversion,
      ))
        ScriptStep(
          id: 'second_wind_conversion_vote',
          title: 'Second Wind Conversion Decision',
          readAloudText:
              "Dealers, The Second Wind has been killed. Do you wish to convert her to become one of you? She will revive as a Dealer, and no one else will die tonight. Vote now: consensus nod if YES, shake head if NO.",
          instructionText:
              "The Dealers vote together on whether to convert The Second Wind. If majority agree (consensus or show of hands), she converts and revives. If they decline, she stays dead.",
          actionType: ScriptActionType.toggleOption,
          roleId: 'dealer',
        ),
      ScriptStep(
      ...
      if (hasWhore)
        ScriptStep(
          ...
          readAloudText:
              "Whore, stay awake for a moment.\n\nPoint to a player. If you or a Dealer are voted out tomorrow, this player will die instead.",
          instructionText:
              "Wait for the Whore to point to a deflection target. Then they close eyes.",
          actionType: ScriptActionType.selectPlayer,
          roleId: 'whore',
        ),
    ];
  ...
  static List<ScriptStep> _buildMedicSteps(List<Player> players) {
    ...
    return const [
      ScriptStep(
        ...
        instructionText:
            "Choose any living player (including yourself) to protect from death tonight. This player cannot be killed by Dealers.",
        actionType: ScriptActionType.selectPlayer,
        roleId: 'medic',
      ),
      ScriptStep(
  ...
  static List<ScriptStep> _buildBouncerSteps(
    ...
    return [
      ScriptStep(
        ...
        title: 'The ID Check',
        readAloudText:
            "Bouncer$creepText$allyText, open your eyes.\n\nBouncer, select a player to I.D.",
        instructionText:
            "Nod if Dealer. Shake head if not. If you I.D. The Minor, she becomes vulnerable to being killed later.",
        actionType: ScriptActionType.selectPlayer,
        roleId: 'bouncer',
      ),
    ...
    if (role.id == 'club_manager') {
      steps.add(
        ScriptStep(
          ...
          readAloudText:
              "Club Manager$creepText, open your eyes.\\n\\nChoose one player's role card to view.\\n\\nClub Manager$creepText, close your eyes.",
          instructionText:
              "Show them the selected player's character card, then they close eyes.",
          actionType: ScriptActionType.selectPlayer,
          roleId: 'club_manager',
        ),
      );
      ...
      steps.add(
        ScriptStep(
          ...
          readAloudText:
              "Wallflower$creepText, open your eyes.\\n\\nIf you wish to witness the murder, raise your hand.\\n\\nWallflower$creepText, close your eyes.",
          instructionText:
              "Optional: If they choose, reveal who the Dealers targeted. Then they close eyes.",
      ...
      steps.add(
        ScriptStep(
          ...
          readAloudText:
              "Sober$creepText, open your eyes.\n\nWho do you wish to save from the night's shenanigans?\n\nSober$creepText, close your eyes.",
          instructionText:
              "That player will not participate tonight and will not be asked to open their eyes. If a Dealer is sent home, no murders happen tonight. Then they close eyes.",
          actionType: ScriptActionType.selectPlayer,
          roleId: 'sober',
        ),
      );
      ...
      steps.add(
        ScriptStep(
          ...
          title: 'Spread Rumour',
          readAloudText:
              "Messy Bitch$creepText, open your eyes.\\n\\nSelect a person to poison with a rumour.\\n\\nMessy Bitch$creepText, close your eyes.",
          instructionText:
              "Track how many rumours have been spread. When all other alive players have rumours, Messy Bitch can convert to an attack dog. Then they close eyes.",
          actionType: ScriptActionType.selectPlayer,
          roleId: 'messy_bitch',
        ),
      );
        ...
        if (clinger.clingerFreedAsAttackDog && !clinger.clingerAttackDogUsed) {
          ...
          steps.add(
            ScriptStep(
              ...
              readAloudText:
                  "Clinger$creepText, open your eyes.\\n\\nIf you have been freed from your obsession (called 'controller'), you may now kill one player. Do you wish to kill? If so, point to your target.\\n\\nClinger$creepText, close your eyes.",
              instructionText:
                  "This ability is only available if Clinger was called 'controller' by their obsession partner in a previous day phase. If they use this, one player dies immediately. Then they close eyes.",
              actionType: ScriptActionType.selectPlayer,
              roleId: 'clinger',
            ),
          );
      ...
      steps.add(
        ScriptStep(
          ...
          instructionText:
              "This step should ideally be merged with Dealer wake.",
          roleId: 'wallflower',
        ),
      );
      ...
      steps.add(
        ScriptStep(
          ...
          readAloudText:
              "Tea Spiller$creepText, open your eyes.\n\nMark a player. If you die, their role will be exposed.\n\nTea Spiller$creepText, close your eyes.",
          instructionText:
              "They choose a target for their potential death reveal.",
          actionType: ScriptActionType.selectPlayer,
          roleId: 'tea_spiller',
        ),
      );
      ...
      steps.add(
        ScriptStep(
          ...
          readAloudText:
              "Lightweight$creepText, open your eyes.\n\n(Host, point to a player now to make their name taboo).\n\nLightweight, nod if you understand.\n\nLightweight$creepText, close your eyes.",
          instructionText:
              "Select the player on screen who you are pointing at in real life to add them to the lightweight's taboo list.",
          actionType: ScriptActionType.selectPlayer,
          roleId: 'lightweight',
        ),
      );
